...
> 最近更新：2026-09-21（深夜）。**133 单机回环方案已终结**：L-2 实测同机
> 两个 URMA 进程并发 urma_init 即双双失败（4096），结合无 ubmad 管理面工具、
> 245 时代 get primary eid failed——本环境 UB 管理面不支持同机多 URMA 进程。
> **当前状态：测试 Agent 待命，等待用户提供「133 同 UB 域 store 节点」的
> 协调结果**。拿到节点后的第一件事 = urma_perftest 基线（133↔store 互通
> 实测，E2 风格），通了才进 SPDK 环节（参考批次 A'' 预案 + env_sop.md）。
> 注意 store 节点可能需要驱动同源（urma_driver 0728ee4 ctrlq 修复届时可用）。
