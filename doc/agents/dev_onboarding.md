...
> 最近更新：2026-09-21。任务书最新指令 = #17 批次 R-0（197 可用性只读侦察），
> 等测试 Agent 回执。R-0 绿灯后下发 R-1（回归 197+151：代码更新 + E1/E2
> 基线实验 + cpu 回归 + npu-staged 全链路首测）。
> 拓扑：197=Initiator（950PR，共享机）+ 151=Target（V100，随时可借）；
> 133/245 已退出（无 UB 链路）。备选：找一台与 133 同 UB 域的 store 节点。
