...
> 最近更新：2026-09-21（晚）。197 确认不可用（退出），245↔151 放弃。
> 当前主力 = **133 单机回环方案**（nvmf_tgt+本地 NVMe+本机另一 udmac，
> 前提四项由批次 L-1 探测），当前指令 = #19 批次 L-1（只读探测），等回执。
> 并行：用户打听与 133 同 UB 域的 store 节点（正解，到手后切批次 A'' 预案）。
